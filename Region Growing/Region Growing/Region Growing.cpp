/*
 ...
 - Formare l'immagine fQ che nel punto (x,y) contiene il valore 1 se Q(f(x,y)) è vero altrimenti contiene il valore 0.
...
int main(int argc, char** argv) {
    ...
    */
    ...
    //Padding è l'etichetta; è un contatore che viene incrementato ogni volta che viene trovata una nuova regione.
    uchar padding = 1;
    ...
    for (int x = 0; x < src.cols; ++x) {
        for (int y = 0; y < src.rows; ++y) {
            /*
              Se dest = 0 vuol dire che il pixel in posizione (x,y) non è stato ancora considerato e quindi è il prossimo seed della regione;
              viceversa vuol dire che già fa parte di una regione e quindi non può essere considerato.
             */
            if (dest.at<uchar>(Point(x, y)) == 0) { 
                ...
                { 
                    ...
                    waitKey();
                    if (++padding > max_region_num) //Se ottengo più di 100 regioni mi fermo, perchè sto over segmentando le immagini.
                    { 
                        printf("Numero di regioni molto alto."); 
                        return -1; 
                    }
                }
                else 
...
void grow(Mat& src, Mat& dest, Mat& mask, Point seed, int threshold2) {
    ...
    //Utilizzo lo stack per simulare la visita in profondità di un grafo.
    stack<Point> point_stack;
    point_stack.push(seed); //Inserisco il seed nello stack.

    while (!point_stack.empty()) { //Continuo la visita finchè lo stack non è vuoto.
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            //Verifico se il punto da considerare è esterno all'immagine, in quel caso si passa al pixel successivo nell'8-intorno:
            ...
            // Se il punto da considerare è interno all'immagine:
            else {
                //Distanza
                ...
                //PREDICATO.
                if (dest.at<uchar>(estimating_point) == 0 //Verifico che estimating_point non è già stato preso in dest, altrimenti vuol dire che appartiene ad un'altra regione;
                    && mask.at<uchar>(estimating_point) == 0 //che non è già stato considerato all'interno di questa regione;
