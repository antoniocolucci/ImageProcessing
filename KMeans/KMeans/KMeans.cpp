...
void myKmeans(Mat& src, Mat& dst, int nClusters)
{
	...
	//Vettore che contiene i pixel che appartengono al cluster.
	...
	for (int k = 0; k < nClusters; k++)
	{
		...
		centers_colors.push_back(center_colors); //Aggiungo lo scalare del centro calcolato al vettore che contiene i colori dei centri.
		//Vettore che conterrà i pixel che verranno assegnati al cluster k.
		vector<Point> t;
		clusters.push_back(t); // Aggiungo un vettore di punti al vettore dei cluster.
	}
	...
	while (difference > 0.1)
	{
		...
		for (int x = 0; x < src.rows; x++)
		{
			for (int y = 0; y < src.cols; y++)
			{
				...
				for (int k = 0; k < nClusters; k++)
				{
					...
					double distance = myDistance(point, center);
		...
		for (int k = 0; k < nClusters; k++)
		{
			...
			//Calcolo la media dei colori del nuovo centro dividendo la somma calcolata prima per il numero di punti.
			blue /= clusterPoints.size();
			green /= clusterPoints.size();
			red /= clusterPoints.size();

			//Estraggo il vecchio centro dal vettore contenente gli scalari dei centri.
			Scalar center = centers_colors[k];
			//Definisco il nuovo centro con le medie calcolate.
			Scalar newCenter(blue, green, red);
			//Calcolo la distanza del nuovo centro al vecchio e l'aggiungo a newCenterSum.
